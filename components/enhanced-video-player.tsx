...
export function EnhancedVideoPlayer({ src, title, startTime = 0, onProgress }: EnhancedVideoPlayerProps) {
  ...
  useEffect(() => {
    ...
    const handleError = () => {
      setVideoError("Failed to load video. Please try refreshing or check the URL.")
      setIsLoading(false)
    }
  ...
  const togglePlay = () => {
    ...
    if (isPlaying) {
      ...
      video.play().catch((error) => {
        ...
        setVideoError("Failed to play video. Try clicking play again.")
      })
    }
  }
  ...
  return (
    ...
    >
          ...
          <div className="text-center max-w-md p-6">
            {videoError ? (
              <div className="text-red-400">
                <div className="text-xl mb-4">⚠️ Video Error</div>
                <div className="text-sm mb-6 text-gray-300">{videoError}</div>
                <Button
                  onClick={() => {
                    setVideoError(null)
                    setIsLoading(true)
      ...
      {/* Controls */}
      <div
        className={`absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/90 to-transparent p-4 transition-opacity duration-300 ${
